...
<div class="space-y-8" data-filter-scope>
    <section class="rounded-[2rem] border border-white/10 bg-white/5 p-8 shadow-xl shadow-black/20 backdrop-blur">
        <div class="mb-6 flex flex-wrap items-center gap-2 text-xs font-semibold uppercase tracking-[0.3em] text-white">
            ...
            <button type="button" data-section-filter="adjustments" class="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-slate-300 transition hover:border-blue-400 hover:text-blue-200">Adjustments</button>
            <button type="button" data-section-filter="watchlist" class="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-slate-300 transition hover:border-blue-400 hover:text-blue-200">Watchlist</button>
            <button type="button" data-section-filter="outbound" class="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-slate-300 transition hover:border-blue-400 hover:text-blue-200">Outbound</button>
        </div>
        <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <h1 class="text-3xl font-bold tracking-tight text-white">Adjustments</h1>
                <p class="mt-2 text-sm text-white">Monitor stock health and record manual corrections.</p>
            </div>
    ...
    <section class="grid gap-8 xl:grid-cols-[1.35fr_0.85fr]">
        <div id="adjustments" data-filter-section="adjustments" class="overflow-hidden rounded-[2rem] border border-white/10 bg-white/5 shadow-xl shadow-black/20 backdrop-blur">
            <div class="border-b border-slate-200 px-6 py-6 dark:border-slate-800">
                <h2 class="text-xl font-semibold text-slate-800 dark:text-white">Recent adjustments</h2>
                <p class="text-sm text-slate-700 dark:text-white">Manual stock corrections recorded by the team</p>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse($adjustments as $adjustment)
                            <tr class="transition hover:bg-slate-50/80 dark:hover:bg-slate-800/60">
                                <td class="px-6 py-5 text-slate-700 dark:text-white">{{ $adjustment->adjustment_date->format('M d, Y') }}</td>
                                <td class="px-6 py-5 font-medium text-slate-800 dark:text-white">{{ $adjustment->batch->item->name ?? '—' }}</td>
                                <td class="px-6 py-5 text-slate-700 dark:text-white">{{ $adjustment->batch->lot_number ?? '—' }}</td>
                                <td class="px-6 py-5 text-center">
                                    <span class="rounded-full px-3 py-1 text-xs font-semibold {{ $adjustment->quantity_changed >= 0 ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-200' : 'bg-rose-100 text-rose-700 dark:bg-rose-500/15 dark:text-rose-200' }}">
                                        {{ $adjustment->quantity_changed > 0 ? '+' : '' }}{{ $adjustment->quantity_changed }}
                                    </span>
                                </td>
                                <td class="px-6 py-5 text-slate-700 dark:text-white">{{ $adjustment->reason }}</td>
                                <td class="px-6 py-5 text-slate-800 dark:text-white">{{ $adjustment->user->name ?? '—' }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-10 text-center text-sm text-slate-700 dark:text-white">No adjustments recorded yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-xl shadow-black/20 backdrop-blur">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="text-sm text-slate-300">Items at or below reorder point</p>
                    </div>
                    <a href="{{ route('items.low-stock') }}" class="text-sm font-medium text-blue-300 transition hover:text-blue-200">View all</a>
                </div>
                        ...
                        <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-slate-300">{{ $item->item_code }}</p>
                                </div>
                                <span class="rounded-full bg-amber-500/15 px-3 py-1 text-xs font-semibold text-amber-200">{{ $stock }} left</span>
                            </div>
                        </div>
                    @empty
                        <div class="rounded-2xl border border-dashed border-white/20 p-6 text-sm text-slate-400">Nothing is below reorder point.</div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-xl shadow-black/20 backdrop-blur">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="text-sm text-slate-300">Batches expiring in 30 days</p>
                    </div>
                    <a href="{{ route('items.near-expiry') }}" class="text-sm font-medium text-blue-300 transition hover:text-blue-200">View all</a>
                </div>
                ...
                <div class="mt-5 space-y-3">
                    @forelse($nearExpiryItems->take(5) as $item)
                        ...
                        <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    <p class="font-semibold text-white">{{ $item->name }}</p>
                                    <p class="mt-1 text-sm text-slate-300">Batch {{ $batch?->lot_number ?? '—' }}</p>
                                </div>
                    ...
                    @empty
                        <div class="rounded-2xl border border-dashed border-white/20 p-6 text-sm text-slate-400">No near-expiry batches right now.</div>
                    @endforelse
                </div>
            </div>
        </aside>
